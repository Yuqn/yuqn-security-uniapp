<!-- <template>
	<societyList :societyListData="societyListData" @data-to-parent="handleDataFromChild" v-if="isOk=='1'"></societyList>
</template>

<script setup lang="ts">
	import societyList from '@/components/society/society-list.vue'
	...
	// 触发组件方法
	function handleDataFromChild(data: { result: string }){
		console.log("data",data);
		let societyId = data.result
		uni.navigateTo({
			url:`/pages/society-list/department-list?societyId=${encodeURIComponent(societyId)}`
		})
	}
	
	onLoad((options)=>{
		let params = reactive({
			"searchValue" : options.societyName
		});
					...
					<view class="collapseItemBodySty">
						<view class="collapseItemBodyViewSty">
							...
							<view class="cardBodyDetailsRowSty">
								<button class="cardBodyUpdateBtnRowSty" @click="clickFun(item.societyId)">查看社团</button>
							</view>
						</view>
					</view>
				</uni-collapse-item>
...
<script lang="ts" setup>
	...
	function clickFun(e){
		uni.navigateTo({
			url:`/pages/society-list/department-list?societyId=${encodeURIComponent(e)}`
		})
	}
