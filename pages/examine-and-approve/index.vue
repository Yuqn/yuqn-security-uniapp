<template>
	<view class="back" :style="{height:height}" style="display: flex; flex-direction: column;">
		...
		<scroll-view scroll-y="true" class="scroll-Y scrollSty" :show-scrollbar="false" >
			...
			<view class="applicationListSty" style="height: 100%;">
						...
						<view v-for="(item,key) in formData" :index="key" :key="item.societyBusiness.id">
							...
							<view v-if="item.societyBusiness.introduce == 'create_society'" style="width: 100%; background-color: white; display: flex;flex-direction: column; height: 200rpx; margin-bottom: 10rpx;">
								...
								<view style="width: 95%; height: 40rpx;margin: 0px auto 0px; display: flex;">
									...
									<view @click="goDetails(item)" style="width: 85rpx; line-height: 40rpx;height: 40rpx; font-size: 0.65rem; color: #6a6a6a; color: #71aaf5;">
										<text>详情-></text>
									</view>
								</view>
								<view style="width: 95%; height: 40rpx;margin: 0px auto 0px; display: flex; ">
							...
							<!-- 新增社团部门 -->
							<view v-if=" item.societyBusiness.introduce == 'create_department'" style="width: 100%; background-color: white; display: flex;flex-direction: column; height: 200rpx; margin-bottom: 10rpx;" @click="goDetails(item)">
								<view style="width: 95%; height: 70rpx;margin: 0px auto 0px; display: flex; border-bottom: 1rpx solid #EBEBEB;">
									<view style="flex-grow: 1;  line-height: 75rpx; height: 70rpx; font-size: 0.85rem;">
										<text>新增社团部门申请</text>
									</view>
							...
							<!-- 编辑社团 -->
							<view v-if="item.societyBusiness.introduce == 'update_society'" style="width: 100%; background-color: white; display: flex;flex-direction: column; height: 200rpx; margin-bottom: 10rpx;" @click="goDetails(item)">
								<view style="width: 95%; height: 70rpx;margin: 0px auto 0px; display: flex; border-bottom: 1rpx solid #EBEBEB;">
									<view style="flex-grow: 1;  line-height: 75rpx; height: 70rpx; font-size: 0.85rem;">
										<text>编辑社团申请</text>
									</view>
						...
						<view v-for="(item,key) in formData" :index="key" :key="item.societyBusiness.id">
								...
								<view v-if="item.societyBusiness.introduce == 'create_society'" style="width: 100%; background-color: white; display: flex;flex-direction: column; height: 200rpx; margin-bottom: 10rpx;">
									...
									<view style="width: 95%; height: 40rpx;margin: 0px auto 0px; display: flex;">
										...
										<view style="width: 85rpx; line-height: 40rpx;height: 40rpx; font-size: 0.65rem; color: #6a6a6a; color: #71aaf5;">
											<text>详情-></text>
										</view>
									</view>
									<view style="width: 95%; height: 40rpx;margin: 0px auto 0px; display: flex; ">
...
<script lang="ts" setup>
	...
	import {getExamineAndApprove,getRejectList,getAllExamineAndApprove} from '../../api/examineAndApprove'
	...
	// 关于切换登录方式的渲染数据
	const items = reactive(['待审批', '已通过', '已拒绝'])
	...
	function getAllExamineAndApproveValue(){
		getAllExamineAndApprove().then(res=>{
			if(res.code == '200'){
				let count = reactive({
	...
	// 获取待审批内容
	function getExamineValue(){
		isOk.value = '0'
		let params = reactive({})
		getExamineAndApprove(params).then(res=>{
			if(res.code == 200){
				formData = res.result
	...
	// 获取已拒绝内容
	function getRejectListValue(e){
		isOk.value = '0'
		let params = reactive({
			"typeId":e
		})
		getRejectList(params).then(res=>{
			if(res.code == '200'){
				console.log("查询成功")
				formData = res.result
				res.result.forEach((item, index, array)=>{
	...
	function onClickItem(e) {
		...
		if(current.value == 1){
			getRejectListValue('1')
		}
		if(current.value == 2){
			getRejectListValue('2')
		}
	}
	...
	function goDetails(e){
		uni.navigateTo({
			url:`/pages/examine-and-approve/examine?businessKey=${encodeURIComponent(JSON.stringify(e.societyBusiness.id))}&procInstId=${encodeURIComponent(JSON.stringify(e.processInstanceId))}&taskId=${encodeURIComponent(JSON.stringify(e.taskId))}`
		})
	}
